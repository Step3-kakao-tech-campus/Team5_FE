...
2. **자동 확장성**: 웹 소켓은 사용자가 많아지면 서버 부하가 증가하고, 이를 관리하기 위해 추가적인 서버 리소스가 필요합니다. API 요청/응답 방식에서도 트래픽이 증가하면 서버 부하가 늘어나고, 이를 관리하기 위해 추가적인 서버 리소스가 필요합니다. 반면 Firebase는 자동으로 확장되므로, 트래픽이 증가하더라도 서버를 유지관리할 필요가 없습니다.
